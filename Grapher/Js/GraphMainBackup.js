    ...
    for (var src = 0; src < Graph.numberOfSelectedNodes; src++) {
        for (var dest = 0; dest < Graph.numberOfSelectedNodes; dest++) {
            ...
            while (!destFound) {
                ...
                for (var adjNodeNum in adjNodes) { //need to something about using adjNode.nodeNum - maybe have nodeNum, nodeLabel, AND gravValue           
                    ...
                    if (!prevVertexes[adjNode.nodeNum]) {
                        var gravDist = Math.abs(/*dest*/destNode.nodeNum - adjNode.nodeNum);//dest needs to be changed to destNode.nodeNum or it will mess up for when I calculate all perms
                        if (gravDist < bestGravDist) {
                            ...
                            if (adjNode.nodeNum < /*dest*/destNode.nodeNum) {
                                bestGravChoice = adjNode.nodeNum;
                            }
                        }
                    }
                ...
                if (equidistanceProblem) {
                    var returnNodeNum = (/*dest*/destNode.nodeNum + bestGravDist);
                ...
                if (Graph.adjacencyMatrix[bestGravChoice][dest] === true || bestGravChoice === /*dest*/destNode.nodeNum) {
                    if (bestGravChoice !== /*dest*/destNode.nodeNum) {
                        currentPath += "," + /*dest*/destNode.nodeNum;
                        currentPathLength++;
                    }
                    if (Graph.shortestPathLengths[src][dest] !== currentPathLength) {
                        ...
                        //return false;
                        //at this point, we know the flavoring isnt gravitational - but we still must check the rest for VPs/LMs if the user desires so
...
Graph.findGravFlavoring = function findGravFlavoring() {
    function permute(level) {
        ...
        for (var i = 0; i < numNodes; ++i) {
            if (used[i]) continue;
            Graph.selectedNodes[indexLoc].nodeNum = initial[i];
            indexLoc++;
            used[i] = true;
            permute(level + 1);
            used[i] = false;
